...
presupuesto_consultoria_datos = _get_state_dict("presupuesto_consultoria_datos", "presupuesto_consultoria")
...
def _mapa_codigo_a_node_id(alcance: dict) -> Dict[str, str]:
    nodos, _, _ = extraer_nodos_desde_edt(alcance)
    mapa = {}
    for nodo in nodos:
        codigo = _safe_str(nodo.get("codigo"))
        node_id = _safe_str(nodo.get("id"))
        if codigo and node_id:
            mapa[codigo] = node_id
    return mapa
...
def _cargar_directos() -> pd.DataFrame:
    pc_items_data = presupuesto_consultoria_datos.get("pc_items_data", {}) or {}
    codigo_a_node = _mapa_codigo_a_node_id(alcance_datos)
    ...
    for filas in pc_items_data.values():
        ...
        for rec in filas:
            ...
            node_id = _safe_str(codigo_a_node.get(item, ""))
            valor_base = _safe_float(rec.get("TOTAL", 0.0), 0.0)

            if valor_base <= 0 or not descripcion or not item or not node_id:
                continue
...
def _cargar_base_actividades() -> pd.DataFrame:
    ...
    if directos.empty:
        return pd.DataFrame(columns=["ROW_ID", "NODE_ID", "ITEM", "TIPO", "DESCRIPCIÓN", "VALOR BASE", "AIU %", "VALOR CON AIU"])

    base = directos.copy().reset_index(drop=True)
    base["_orden_item"] = base["ITEM"].apply(_key_codigo_natural)
    base = base.sort_values(by=["_orden_item", "DESCRIPCIÓN"], ascending=[True, True]).reset_index(drop=True)
    return base.drop(columns=["_orden_item"], errors="ignore")
...
# ==========================================================
def _estado_key() -> str:
    return "flujo_fondos_consultoria"
...
# UI
# ==========================================================
st.set_page_config(page_title="FLUJO DE FONDOS CONSULTORIA", layout="wide")
st.title("FLUJO DE FONDOS CONSULTORIA")

nombre_proyecto = _safe_str(alcance_datos.get("nombre_proyecto", "")) or "SIN NOMBRE DEFINIDO"
...
if tipo_presupuesto_proyecto != "Consultoría":
    st.warning("Este proyecto está clasificado como Obra. La hoja Flujo de Fondos Consultoría aplica solo para proyectos de Consultoría.")
    st.stop()

base_df = _cargar_base_actividades()
if base_df.empty:
    st.warning("No hay actividades con costo disponibles en el presupuesto de consultoría.")
    st.stop()
...
seleccion = st.selectbox(
    ...
    key="flujo_fondos_consultoria_editor_selector",
)
...
if not row_sel_df.empty:
    ...
    editor_df_edit = st.data_editor(
        ...
        hide_index=True,
        num_rows="fixed",
        key="flujo_fondos_consultoria_editor_superior",
...
st.data_editor(
    ...
    hide_index=True,
    num_rows="fixed",
    key="flujo_fondos_consultoria_pct_tabla_general",
...
st.data_editor(
    ...
    hide_index=True,
    num_rows="fixed",
    key="flujo_fondos_consultoria_valores",
...
st.data_editor(
    ...
    hide_index=True,
    num_rows="fixed",
    key="flujo_fondos_consultoria_resumen",
...
st_echarts(
    ...
    height="460px",
    width="100%",
    key="flujo_fondos_consultoria_echarts",
)
