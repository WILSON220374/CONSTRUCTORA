...
def _get_any(d, *keys):
    if not isinstance(d, dict):
        return ""
    for key in keys:
        if key in d and _texto(d.get(key)):
            return d.get(key)
    return ""
...
def _fecha_vencimiento_inicial(acta_inicio, contrato_obra):
    ...
    plazo_dias = int(_safe_float(acta_inicio.get("plazo_ejecucion_dias"), 0.0) or 0)

    if plazo_dias <= 0:
        plazo_dias = int(_safe_float(contrato_obra.get("plazo_ejecucion_dias"), 0.0) or 0)
...
def _aiu_desde_presupuesto(presupuesto_obra):
    ...
    administracion = _safe_float(_get_any(config, "administracion", "administracion_pct", "a_pct"), 0.0)
    imprevistos = _safe_float(_get_any(config, "imprevistos", "imprevistos_pct", "i_pct"), 0.0)
    utilidad = _safe_float(_get_any(config, "utilidad", "utilidad_pct", "u_pct"), 0.0)
    aiu_total = _safe_float(_get_any(config, "aiu_pct_global", "aiu_total"), 0.0)
    if aiu_total <= 0:
...
# Presupuesto / ítems
# ==========================================================
def _extraer_tablas_presupuesto(presupuesto_obra):
    ...
    for nombre in [
        ...
        valor = tablas.get(nombre) or presupuesto_obra.get(nombre) or []
        if isinstance(valor, list):
            fuentes.append(valor)

    flujo_directos = presupuesto_obra.get("flujo_fondos_directos", []) or []
    if isinstance(flujo_directos, list):
        fuentes.append(flujo_directos)

    grupos = tablas.get("grupos_presupuesto_obra", []) or presupuesto_obra.get("grupos_presupuesto_obra", []) or []
    if isinstance(grupos, list):
        for grupo in grupos:
            if isinstance(grupo, dict) and isinstance(grupo.get("rows"), list):
                fuentes.append(grupo.get("rows"))
...
def _items_desde_presupuesto(presupuesto_obra):
    ...
    for fuente in _extraer_tablas_presupuesto(presupuesto_obra):
        for fila in fuente:
            if not isinstance(fila, dict):
                continue

            item = _primero_no_vacio(
                fila.get("ITEM"),
                fila.get("ITEM GOBER"),
                fila.get("No. ORDEN"),
                fila.get("No ORDEN"),
            ...
            descripcion = _primero_no_vacio(
                ...
                fila.get("DESCRIPCION ITEM"),
            )
            if not item and not descripcion:
                continue

            item = item or descripcion
            rec = registros.get(
                item,
                {
                    "No. ORDEN": item,
            ...
            if not rec.get("DESCRIPCIÓN ITEM"):
                rec["DESCRIPCIÓN ITEM"] = descripcion
    ...
    return sorted(registros.values(), key=lambda x: _key_codigo_natural(x.get("No. ORDEN")))
...
def _normalizar_resumen_financiero(rows):
    ...
    for fila in rows or []:
        if not isinstance(fila, dict):
            continue
        salida.append({k: (round(_safe_float(fila.get(k), 0.0), 2) if k != "MES" else _texto(fila.get(k))) for k in [
            "ACTA No.", "MES",
            "ACTAS DE RECIBO PARCIAL - VALOR BÁSICO", "ACTAS DE RECIBO PARCIAL - VALOR IVA", "ACTAS DE RECIBO PARCIAL - VALOR TOTAL",
            "AJUSTES PROVISIONALES - VALOR BÁSICO", "AJUSTES PROVISIONALES - VALOR IVA", "AJUSTES PROVISIONALES - VALOR TOTAL",
            "AJUSTES DEFINITIVOS - VALOR BÁSICO", "AJUSTES DEFINITIVOS - VALOR IVA", "AJUSTES DEFINITIVOS - VALOR TOTAL",
        ]})
    return salida
...
def _doc_tabla_df(doc, df, columnas=None):
    ...
    if not columnas:
        ...
        df = pd.DataFrame([{"Información": "Sin registros"}])
        ...
        for i, col in enumerate(columnas):
            ...
            if isinstance(val, (date, datetime, pd.Timestamp)):
                ...
                if "VALOR" in str(col).upper():
                    ...
                    val = _numero(val, 4 if "CANTIDAD" in str(col).upper() else 2)
...
def _generar_word(payload):
    ...
    for titulo, key in [
        ...
        ("RELACIÓN SUSPENSIONES Y AMPLIACIONES DE SUSPENSIÓN", "suspensiones"),
    ]:
        doc.add_paragraph()
        _doc_parrafo(doc, titulo, bold=True)
        _doc_tabla_df(doc, pd.DataFrame(payload.get(key, [])))

    doc.add_paragraph()
    _doc_parrafo(doc, "DESCRIPCIÓN GENERAL DE LA OBRA EJECUTADA", bold=True)
    _doc_parrafo(doc, "LOCALIZACIÓN Y LONGITUD DE LA META FÍSICA EJECUTADA:", bold=True)
    ...
    _doc_parrafo(doc, "RELACIÓN DE SITIOS CRÍTICOS ATENDIDOS Y/O SEÑALIZADOS:", bold=True)
    ...
    for titulo, key in [
        ("CONCEPTO DE LA INTERVENTORÍA RECIBO DE LAS OBRAS", "concepto_interventoria"),
        ("CONCEPTO DE LA INTERVENTORÍA SOBRE LA APROBACIÓN DE LOS PLANOS RECORD", "concepto_planos_record"),
        ("GESTIÓN AMBIENTAL, SOCIAL, PREDIAL Y DE SOSTENIBILIDAD", "gestion_ambiental_social_predial"),
        ("OTRAS ACTIVIDADES APROBADAS POR EL INTERVENTOR", "otras_actividades"),
        ("OBSERVACIONES DEL INTERVENTOR RESPECTO DEL ESTADO DE LAS GARANTÍAS Y SEGUROS CONTRACTUALES EXIGIDOS AL CONTRATISTA DE OBRA", "observaciones_garantias"),
        ("OBSERVACIONES", "observaciones_generales"),
    ]:
        doc.add_paragraph()
        _doc_parrafo(doc, titulo, bold=True)
        _doc_parrafo(doc, payload.get(key, ""))
...
anticipio_valor_contrato = generales.get("valor_inicial", 0.0)
anticipo = _anticipo_desde_fuentes(plan_anticipo, control_obra, anticipio_valor_contrato)
...
localizacion_meta = st.text_area("LOCALIZACIÓN Y LONGITUD DE LA META FÍSICA EJECUTADA", value=guardado.get("localizacion_meta", ""), height=90)
...
sitios_criticos = st.text_area("RELACIÓN DE SITIOS CRÍTICOS ATENDIDOS Y/O SEÑALIZADOS", value=guardado.get("sitios_criticos", ""), height=90)

st.markdown("### VALOR TOTAL EJECUTADO DEL CONTRATO DE OBRA")
valor_total_ejecutado = st.number_input("Valor total ejecutado del contrato", value=float(_safe_float(guardado.get("valor_total_ejecutado"), 0.0)), min_value=0.0, step=1000.0, format="%.2f")
...
df_resumen = pd.DataFrame(_filas_resumen_financiero_guardadas(guardado), columns=cols_resumen)
resumen_editado = st.data_editor(df_resumen, hide_index=True, width="stretch", num_rows="dynamic", key="resumen_financiero_definitivo_editor")
...
gestion_ambiental_social_predial = st.text_area("GESTIÓN AMBIENTAL, SOCIAL, PREDIAL Y DE SOSTENIBILIDAD", value=guardado.get("gestion_ambiental_social_predial", ""), height=140)
otras_actividades = st.text_area("OTRAS ACTIVIDADES APROBADAS POR EL INTERVENTOR", value=guardado.get("otras_actividades", ""), height=90)
