...
def fecha_en_letras_larga(valor):
    ...
    for fmt in formatos:
        try:
            ...
            return f"a los {f.day} dias del mes de {meses[f.month]} de {_numero_a_letras_basico(f.year)}"
        except Exception:
            continue
...
def construir_contrato(datos):
    ...
    fecha_contrato = fecha_en_letras_larga(datos.get("fecha_contrato"))
...
def generar_word(datos):
    ...
    for bloque in contrato_txt.split("\n\n"):
        ...
        elif "| Amparo | Suficiencia | % | Cobertura | Desde | Hasta |" in bloque:
