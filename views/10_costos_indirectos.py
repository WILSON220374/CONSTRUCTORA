...
if tipo_presupuesto_proyecto != "Obra":
    st.warning("Este proyecto está clasificado como Consultoría. La hoja Costos Indirectos aplica solo para proyectos de Obra.")
    st.stop()
...
try:
    presupuesto_obra_datos = st.session_state.get("presupuesto_obra_datos") or cargar_estado("presupuesto_obra") or {}
except Exception:
    presupuesto_obra_datos = st.session_state.get("presupuesto_obra_datos", {}) or {}

try:
    estudio_mercado_datos = cargar_estado("estudio_mercado") or {}
except Exception:
    estudio_mercado_datos = {}

otros_costos_indirectos = alcance.get("otros_costos_indirectos_proyecto", []) or []
...
st.markdown("### Costo indirecto origen")

costo_indirecto_origen = st.selectbox(
    "Seleccione el costo indirecto del presupuesto de obra",
    options=opciones_oci,
    index=None,
    placeholder="Seleccione un costo indirecto",
    key="ci_costo_indirecto_origen",
)

if not costo_indirecto_origen:
    st.info("Seleccione un costo indirecto del presupuesto de obra para cargar la información.")
    st.stop()
...
registros_por_oci_original = costos_indirectos_datos.get("registros_por_oci", {}) or {}
...
costos_indirectos_datos["registros_por_oci"] = registros_por_oci
st.session_state["costos_indirectos_datos"] = costos_indirectos_datos
...
if cambio_estudio:
    estudio_mercado_datos = estudio_mercado_limpio
    for key_tmp in [
        "em_items_data",
...
if cambio_costos or cambio_estudio:
    try:
        if cambio_costos:
            guardar_estado("costos_indirectos", costos_indirectos_datos)
        if cambio_estudio:
            guardar_estado("estudio_mercado", estudio_mercado_datos)
    except RuntimeError as e:
        if "No se pudo refrescar la sesión" in str(e):
            st.warning("La sesión expiró. Inicia sesión de nuevo para guardar cambios en costos indirectos.")
        else:
            raise
...
if (
    ...
    st.session_state["costos_indirectos_iva_bienes_pct"] = float(
        registro_actual_oci.get("iva_bienes_pct", 19.0) or 19.0
    )
    st.session_state["costos_indirectos_iva_servicios_pct"] = float(
        registro_actual_oci.get("iva_servicios_pct", 19.0) or 19.0
    )
    ...
    st.session_state["ci_resumen_final_data"] = registro_actual_oci.get(
        "resumen_final_data",
        [
            {"Concepto": "Costo personal", "IVA": "No"},
            {"Concepto": "Bienes", "IVA": "Sí"},
            {"Concepto": "Servicios", "IVA": "Sí"},
        ],
    )
...
df_em_items = pd.DataFrame(estudio_mercado_datos.get("items", [])).copy()
df_em_cot = pd.DataFrame(estudio_mercado_datos.get("cotizaciones", [])).copy()
...
if "costos_indirectos_iva_bienes_pct" not in st.session_state:
    st.session_state["costos_indirectos_iva_bienes_pct"] = float(
        registro_actual_oci.get("iva_bienes_pct", 19.0) or 19.0
    )

if "costos_indirectos_iva_servicios_pct" not in st.session_state:
    st.session_state["costos_indirectos_iva_servicios_pct"] = float(
        registro_actual_oci.get("iva_servicios_pct", 19.0) or 19.0
    )
...
def _guardar_costos_indirectos():
    try:
        payload_actual = costos_indirectos_datos.copy() if isinstance(costos_indirectos_datos, dict) else {}
        ...
        costo_directo_total = total_personal + subtotal_bienes + subtotal_servicios

        iva_pct_guardado = float(st.session_state.get("costos_indirectos_iva_bienes_pct", 19.0) or 19.0)
        resumen_guardado = st.session_state.get("ci_resumen_final_data", []) or []
        resumen_guardado_map = {
            str(row.get("Concepto", "") or "").strip(): str(row.get("IVA", "No") or "No").strip()
            for row in resumen_guardado
            if str(row.get("Concepto", "") or "").strip()
        }

        valores_resumen_guardado = [
            ("Costo personal", total_personal),
            ("Bienes", subtotal_bienes),
            ("Servicios", subtotal_servicios),
        ]

        valor_total_final = 0.0
        for concepto_tmp, valor_tmp in valores_resumen_guardado:
            aplica_iva_tmp = resumen_guardado_map.get(concepto_tmp, "No")
            valor_total_final += float(valor_tmp or 0.0) * (
                1 + iva_pct_guardado / 100.0 if aplica_iva_tmp == "Sí" else 1.0
            )

        iva_total = valor_total_final - costo_directo_total
        ...
        if oci_id:
            registros_actuales[oci_id] = {
                ...
                "bienes": bienes_data,
                "iva_bienes_pct": float(st.session_state.get("costos_indirectos_iva_bienes_pct", 19.0) or 19.0),
                "servicios": servicios_data,
                "iva_servicios_pct": float(st.session_state.get("costos_indirectos_iva_servicios_pct", 19.0) or 19.0),
                "resumen_final_data": st.session_state.get("ci_resumen_final_data", []),
        ...
        guardar_estado("costos_indirectos", payload_actual)
        st.session_state["costos_indirectos_datos"] = payload_actual
        return True
    except RuntimeError as e:
        if "No se pudo refrescar la sesión" in str(e):
            st.warning("Sesión expirada. Inicia sesión de nuevo para guardar cambios en costos indirectos.")
            return False
        raise


if st.button("Guardar costos indirectos", width="stretch"):
...
def _normalizar_df_personal(df: pd.DataFrame, prefijo_item: str) -> pd.DataFrame:
    ...
    for idx in df.index:
        ...
        item_gober = str(df.at[idx, "ITEM GOBERNACION"]).strip()
        desc_gober = str(df.at[idx, "DESCRIPCION GOBERNACION"]).strip() if "DESCRIPCION GOBERNACION" in df.columns else ""
        seleccionar_gober = str(df.at[idx, "SELECCIONAR GOBER"]).strip()

        if fuente == "Precios gobernación":
            if seleccionar_gober:
                codigo_sel = seleccionar_gober.split(" | ", 1)[0].strip()
                if codigo_sel in map_item_gober:
                    ref = map_item_gober[codigo_sel]
                    df.at[idx, "ITEM GOBERNACION"] = codigo_sel
                ...
                df.at[idx, "ITEM GOBERNACION"] = ref["item_gober"]
                df.at[idx, "UNIDAD"] = ref["unidad"]
                df.at[idx, "VR UNITARIO"] = ref["vr_unitario"]
    ...
    st.data_editor(
        ...
        column_config={
            "SELECCIONAR GOBER": st.column_config.SelectboxColumn(
                "SELECCIONAR GOBER",
                options=[""] + [f"{k} | {v['descripcion_gober']}" for k, v in map_item_gober.items()],
            ),
            "ITEM": st.column_config.TextColumn("ITEM", disabled=True),
            "ITEM GOBERNACION": st.column_config.TextColumn("ITEM GOBERNACION", disabled=True),
        ...
        disabled=["ITEM", "UNIDAD", "VR UNITARIO", "VR TOTAL"],
    )
...
def _render_tabla_bs(titulo: str, key_data: str, key_widget: str, prefijo_item: str, key_guardado: str):
    ...
    if isinstance(edited_df, pd.DataFrame) and not edited_df.empty:
        df_final = _normalizar_df_bs(edited_df, prefijo_item)
        st.session_state[key_data] = df_final.to_dict(orient="records")
    else:
...
iva_bienes_pct_actual = float(st.session_state.get("costos_indirectos_iva_bienes_pct", 19.0) or 19.0)
iva_bienes_valor = subtotal_bienes * (iva_bienes_pct_actual / 100.0)
total_bienes = subtotal_bienes + iva_bienes_valor

iva_bienes_valor = subtotal_bienes * (float(st.session_state.get("costos_indirectos_iva_bienes_pct", 19.0) or 19.0) / 100.0)
total_bienes = subtotal_bienes + iva_bienes_valor
...
iva_servicios_valor = subtotal_servicios * (
    float(st.session_state.get("costos_indirectos_iva_bienes_pct", 19.0) or 19.0) / 100.0
)
total_servicios = subtotal_servicios + iva_servicios_valor

cs3, cs4 = st.columns([5, 1])
with cs3:
    st.markdown("## TOTAL SERVICIOS")
with cs4:
    st.markdown(f"## $ {total_servicios:,.2f}")
...
st.number_input(
    ...
    step=0.01,
    format="%.2f",
    key="costos_indirectos_iva_bienes_pct",
)
...
iva_pct_resumen = float(st.session_state.get("costos_indirectos_iva_bienes_pct", 19.0) or 19.0)

iva_pct_resumen = float(st.session_state.get("costos_indirectos_iva_bienes_pct", 19.0) or 19.0)

iva_pct_resumen = float(st.session_state.get("costos_indirectos_iva_bienes_pct", 19.0) or 0.0)
...
resumen_base_df["VALOR TOTAL"] = resumen_base_df.apply(
    lambda r: float(pd.to_numeric(r["Valor"], errors="coerce") or 0.0) * 1.19
    if str(r["IVA"] or "").strip() == "Sí"
    else float(pd.to_numeric(r["Valor"], errors="coerce") or 0.0),
    axis=1,
)
