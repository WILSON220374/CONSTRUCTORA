...
if tipo_presupuesto_proyecto != "Obra":
    st.warning("Este proyecto está clasificado como Consultoría. La hoja Estudio de Mercado aplica solo para proyectos de Obra.")
    st.stop()
...
def _ids_oci_vigentes() -> set[str]:
    costos_indirectos_vigentes = alcance.get("otros_costos_indirectos_proyecto", []) or []
    return {
        str(ci.get("id", "") or "").strip()
        for ci in costos_indirectos_vigentes
        if str(ci.get("id", "") or "").strip()
    }


def _extraer_items_hoja_10(datos: dict) -> list[dict]:
    items = []

    registros_por_oci = datos.get("registros_por_oci", {}) or {}
    ids_costos_indirectos_vigentes = _ids_oci_vigentes()
...
def _sincronizar_estado_con_hoja_10(guardar: bool = False):
    global costos_indirectos_datos, estudio_datos

    ids_vigentes = _ids_oci_vigentes()
    registros_filtrados = {
        str(oci_id): registro
        for oci_id, registro in (costos_indirectos_datos.get("registros_por_oci", {}) or {}).items()
        if str(oci_id).strip() in ids_vigentes
    }

    if registros_filtrados != (costos_indirectos_datos.get("registros_por_oci", {}) or {}):
        costos_indirectos_datos["registros_por_oci"] = registros_filtrados
        st.session_state["costos_indirectos_datos"] = costos_indirectos_datos
        if guardar:
            guardar_estado("costos_indirectos", costos_indirectos_datos)

    items_origen = _extraer_items_hoja_10(costos_indirectos_datos)

    items_base = st.session_state.get(key_items, estudio_datos.get("items", []))
    cot_base = st.session_state.get(key_cot, estudio_datos.get("cotizaciones", []))
    ...
    if guardar or estudio_limpio != {
        "items": estudio_datos.get("items", []),
        "cotizaciones": estudio_datos.get("cotizaciones", []),
    }:
        estudio_datos = estudio_limpio
        if guardar:
            guardar_estado("estudio_mercado", estudio_datos)
...
def _guardar_estudio_mercado():
    try:
        items_actuales_hoja10 = _extraer_items_hoja_10(costos_indirectos_datos)
...
def _aplicar_a_hoja_10():
    try:
        ...
        try:
            costos = cargar_estado("costos_indirectos") or {}
        except Exception:
            costos = {}
        ...
        guardar_estado("costos_indirectos", costos)
        st.session_state["costos_indirectos_datos"] = costos
        st.success("Promedios aplicados a la hoja 10 correctamente.")
        return True

    except Exception:
        st.error("No fue posible aplicar los promedios a la hoja 10.")
        return False
...
def _construir_render_desde_estado(items_df_base: pd.DataFrame, cot_df_base: pd.DataFrame, num_cotizaciones: int) -> pd.DataFrame:
    ...
    for _, row in resumen_df_base.iterrows():
        ...
        fila = {
            ...
            "COSTO_INDIRECTO_ORIGEN_ID": str(row.get("COSTO_INDIRECTO_ORIGEN_ID", "") or ""),
            "COSTO_INDIRECTO_ORIGEN_NOMBRE": str(row.get("COSTO_INDIRECTO_ORIGEN_NOMBRE", "") or ""),
...
for tipo, titulo in orden_tipos:
    ...
    if items_sec.empty:
        st.info(f"No hay registros de {titulo.lower()} con fuente Cotización en la hoja 10.")
        st.divider()
        continue
...
with b2:
    if st.button("Asignar costos", width="stretch"):
        ok_guardado = _guardar_desde_editores(int(st.session_state.get("em_num_cotizaciones", 3) or 3))
        if ok_guardado:
            _aplicar_a_hoja_10()
